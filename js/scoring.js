...
export function emptyEntry() {
  return { numbers: [], plus: [], doubled: false, busted: false, frozen: false, manual: null, flip7: false };
}
...
export function computeRound(entry = {}) {
  ...
  if (busted) {
    return { total: 0, base: 0, doubledBase: 0, bonus: 0, flip7: false, busted: true, frozen: false, numbers, plus, doubled, cards: numbers.length };
  }
  ...
  return { total, base, doubledBase, bonus, flip7, busted: false, frozen, numbers, plus, doubled, cards: numbers.length };
}
...
export function formulaOf(entry) {
  ...
  const frozen = (text) => (r.frozen ? `${text} · congelato` : text);
  if (r.manual) return frozen(r.flip7 ? `${r.typed} + 15 Flip 7` : "punti inseriti a mano");
  if (!r.numbers.length && !r.plus.length) return r.frozen ? "congelato senza carte" : "nessuna carta";
  const parts = [];
